...
class LLMManager:
    ...
    Priority: Groq -> Gemini -> Ollama -> Mock (Development)
    """
    ...
    async def generate(self, prompt: str) -> str:
        ...
        try:
            ...
            logger.warning(f"Ollama failure: {e}. Using Mock LLM for testing/development.")

        # 4. Use Mock LLM (always available for testing/development)
        try:
            logger.info("Attempting generation with Mock LLM (Development Fallback)")
            return await self.mock_service.generate(prompt)
        except Exception as e:
            logger.error(f"All LLM providers failed including Mock. Final error: {e}")
